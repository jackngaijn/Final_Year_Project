...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.65\textwidth]{../Pic/pic1.png} % Replace "example-image" with your image file
...
\subsubsection{RNN encoder decoder model}
\begin{figure}[!htb]
    \centering
    \includegraphics[width=1\textwidth]{../Pic/RNN_encoder_decoder.jpg} % Replace "example-image" with your image file
\end{figure}
RNN Encoder-Decoderis a RNN based architecture which commonly used for handle sequence to sequence problem, for example machine translation, summarization, and question answering. The designed of this architecture is good for handle the input sequence and the output sequence is not the same length, leveraging the sequential nature of data. 
\\[1ex]
...
\begin{longtable}{|c|c|c|}
    ...
    \textbf{Notation} & \textbf{Meaning} & \textbf{Dimension}\\
    \hline
    $W^{enc}_{ih}$          & Weight matrix for input to hidden state in encoder      & $hidden\ units\times input\ size$\\
    $W^{enc}_{hh}$          & Weight matrix for hidden to hidden state in encoder     & $hidden\ units\times hidden\ units$\\
    $b^{enc}_{h}$           & Bias vector for the hidden state in encoder             & $hidden\ units\times hidden\ units$\\
    $h^{enc}_{t}$           & Hidden state at timestep t in encoder                   & $hidden\ units\times hidden\ units$\\
    \hline
    \caption{Encoder}
\end{longtable}
After the model processed the entire input sequence from $t=1$ to $t=T$, the last hidden state $h^{enc}_T$ treat as the context vector $c$. 
\begin{equation}
    c = h^{enc}_T
\end{equation}
\textbf{Decoder}
\\[1ex]
The decoder is the second component of RNN encoder decoder, the usage of the decoder is to generates the output sequence step by step, conditioned on the context vector $c$ and its previous hidden state. At each timestep $t$, the decoder updates its hidden state $s_t'$ based on the previous hidden state $s_{t'-1}$ and the embedding of the previous output $y_{t'-1}$. And we will define the $s_0 = c$ first.
\begin{equation}
    h^{dec}_t = tanh(W^{dec}_{ih}y_{t-1}+W^{dec}_{hh}+b^{dec}_h)
\end{equation}
After the model computed the $h^{dec}_t$, the model will compute the output probability distribution.
\begin{equation}
    P(y_t|y_{t-1}, c) = softmax(W_oh^{dec}_t+b^{dec}_o)
\end{equation}
And the predicted token $y_t = arg maxP(y_t)$, the process will repeat until the decoder predicts the end of sequence token $(<EOS>)$.
\begin{longtable}{|c|c|c|}
    \hline
    \textbf{Notation} & \textbf{Meaning} & \textbf{Dimension}\\
    \hline
    $W^{dec}_{ih}$          & Weight matrix for input to hidden      & $hidden\ units\times input\ size$\\
    $W^{dec}_{hh}$          & Weight matrix for hidden to hidden     & $hidden\ units\times hidden\ units$\\
    $b^{dec}_{h}$           & Bias vector for the hidden              & $hidden\ units\times hidden\ units$\\
    $h^{dec}_{t}$           & Hidden state at timestep t                    & $hidden\ units\times hidden\ units$\\
    $b^{dec}_{o}$           & Bias vector for the output layer             & $hidden\ units\times hidden\ units$\\
    $W^{dec}_{o}$           & Weight matrix for hidden to output     & $output\ feature\ space\times hidden\ units$\\
    \hline
    \caption{Decoder}
\end{longtable}
...
% Vaswani et al. introduces the Transformer architecture, which replaces RNN model with self attention. In this section, I want to review the attention machanism. \parencite{vaswani2023attentionneed}. Attention machanism is good for handle the sequence-to-sequence tasks such as machine translation, text summarization and image captioning. Bahdanau et al. \parencite{bahdanau2016neuralmachinetranslationjointly} introduces the attention mechanism in the machine translation task. At each decoding step $t'$, the model model will compute the attention scores $\alpha_{t',t}$ that represent how important of the current hidden state $h_t$ to the current decoder state $s_{t'}$. We have different attention formula. 

\textbf{Bahdanau (Additive) Attention}
\\[1ex]
This method proposed by \parencite{bahdanau2016neuralmachinetranslationjointly}, in which this attention uses a feedforward network to compute the attention scores. 
\\[1ex]
\begin{equation}
    e_{t',t} = v_a^Ttanh(W_ah^{dec}_{t'-1}+U_ah^{enc}_t+b_a)
\end{equation}                    
\textbf{Luong (Multiplicative) Attention}
\\[1ex]
This method is introduced by Luong et al \parencite{luong2015effectiveapproachesattentionbasedneural}. This method uses the dot product to compute the attention scores.
\begin{equation}
    e_{t',t} = h^{dec}_{t'-1} \cdot h^{enc}_t
\end{equation}
\textbf{Scaled Dot-Product Attention}
\\[1ex]
The scaled dot-product attention was introduced by \parencite{vaswani2017attentionneed}, this is the key component in Transformer architecture. It improves from the Luong's dot-product attention \parencite{luong2015effectiveapproachesattentionbasedneural} by addressing he numerical stability issues when working with the high dimensional vectors and making attention computation more efficient for parallelization. Below is the formula of attention scores.
\begin{equation}
    S = \dfrac{QK^T}{\sqrt{d_k}}
\end{equation}
Where $S \in \mathbb{R}^{n \times n}$ is pairwise similarity scores between all tokens. 


\subsubsection{Hidden Markov Model}
Before reviewing Hidden Markov Model (HMM), it is essential to understand what Markov models is, or Markov chains. Markov chains are fundamental models in probability theory and statistic, and it is a stochastic process, which describe a sequence of possible events in which the probability of each event depends only on the state attained in the previous event. This property known as the Markov property \parencite{roberts2004general, rabiner1986anintroduction}. In rigorous terms. Let the state space be defined as:
\begin{equation}
    S = \{ s_1, s_2, \dots, s_N \}.
\end{equation}
\begin{equation}
...
\begin{itemize}
    ...
    \item RNN Encoder-Decoder with Attention Mechanism
\end{itemize}
...
Compare different RNN-based models on metrics relevant to the target NLP taskquantitatively and qualitatively. For example accuracy.
...
\subsubsection{Model 5 (Gated recurrent unit with 2 layer)}
Same setting with model 1, but I stack 2 GRU block. In the other word, this is deep recurrent neural networks. 
\subsubsection{Model 6 (Long short-term memory with 2 layer)}
Same setting with model 1, but I stack 2 LSTM block. In the other word, this is deep recurrent neural networks. 

\subsubsection{Model 7 (RNN Encoder-Decoder)}
For the RNN encoder-decoder architecture, I will explain how to data be processes and I will show the mathmatical transformations for each time step. We first assume the input $X = [x_1, x_2, \cdots, x_T]$ be the input sequence and the $T$ is the sequence length. $x$ is word index in the vocabulary. 
\\[1ex]
...
\begin{equation}
    h_t = tanh(W_{ih}e_t+W_{hh}h_{t-1}+b_h)
\end{equation}
Where $W_{ih} \in \mathbb{R}^{hidden\ dim \times embedding\ dim}$, $W_{hh} \in \mathbb{R}^{hidden\ dim \times hidden\ dim}$. $b_h \in \mathbb{R}^{hidden\ dim}$.
\\[1ex]
...
\subsubsection{Model 8 (RNN Encoder-Decoder with attention mechanism)}
...
\begin{itemize}
    \item \textbf{Architecture Hyperparameters:}
    \begin{itemize}
        \item Number of layers: \texttt{no\_layers = 2}
        \item Vocabulary size: \(\mathtt{vocab\_size = len(vocab) + 1}\) (the extra one is for unknown word)
        \item Embedding dimension: \texttt{embedding\_dim = 100}
        \item Hidden dimension: \texttt{hidden\_dim = 50}
        \item Output dimension: \texttt{output\_dim = 1}
...
\begin{itemize}
    ...
    In this project, a stratified k-fold cross validation was applied as part of the training loop to evaluate and compare the performance of various RNN-based model. And I will use the better performance of the best combination which has the highest mean accuracy. And this method was designed to ensure that each model was trained and validated on all data splits, providing a more comprehencive evaluation of model performance.
    \\[1ex]
...
\subsubsection{Model 7 (RNN encoder-decoder)}
\textbf{Loss over epochs (left plot)}
\begin{enumerate}
    \item Training Loss:
    \begin{itemize}
        \item The training loss consistently decreases over the epochs, which mean that the model is leaning and fitting the training data better as training progresses.
        \item By the end of the epoch, which mean 10th epoch, the training loss drops significantly from roughtly 0.63 to 0.55.
    \end{itemize}
    \item Test Loss:
    \begin{itemize}
        \item The test loss shows more fluctuation compared to the training loss, initially, the test loss is greater that the training loss, and this is expected. After that, it decreases slightly in the first few epochs but then starts to fluctuate significantly, with a noticeable spike around epoch 6 and another drop by epoch 9. At the end of the test loss is around 0.725.
    \end{itemize}
\end{enumerate}
\textbf{Accuracy Over Epochs (Right plot)}
\begin{enumerate}
    \item Training Accuracy:
    \begin{itemize}
        \item The training accuracy steadily increases from roungthly 60\% in the first epoch to above 72.5\% by the 10th epoch. 
    \end{itemize}
    \item Test Accuracy:
    \begin{itemize}
        \item Test accuracy initially improves, peaking at epoch 4 at approximately 70\%, after epoch 4, the test accuracy drop dramatically to below 60\%.
    \end{itemize}
\end{enumerate}
\begin{figure}[!htb]
    \centering
    \includegraphics[width=1\textwidth]{../Pic/RNN_encoder_decoder_1_layer_performance.png}
    \caption{LSTM with 2 layer performance on IMDB dataset}
\end{figure}
...
\begin{enumerate}
    \item Training Loss:
    \begin{itemize}
        \item The training loss cansistently decreases throughout the eopchs, starting from around 0.65 and dropping to below 0.3 by the 10th eopch. Which mean that the model is leaning effectively and improving its performance on the training data as training progresses.
    \end{itemize}
    \item Test Loss:
    \begin{itemize}
        \item The test loss fluctuates more than the training loss, it increases slightly in the first few epochs and peaking at around epoch 4 with a value near 0.8. At the end of the epoch, the loss drop back to around 0.6.
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    \item Training Accuracy:
    \begin{itemize}
        \item The training accuracy steadily increases over the epochs, starting from around 60\% and raise to 90\% by the end of epoch. This demonstrates that the model becomes increaseingly proficient at predicting the correct labels for the training data.
    \end{itemize}
    \item Test Accuracy:
    \begin{itemize}
        \item The test accuracy steadily increase from 72\% to 85\%. 
    \end{itemize}
\end{enumerate}
\begin{figure}[!htb]
    \centering
    \includegraphics[width=1\textwidth]{../Pic/RNN_encoder_decoder_with_attention_1_layer_performance.png}
    \caption{LSTM with 2 layer performance on IMDB dataset}
\end{figure}
\newpage

\subsubsection{Word Importancy}
\textbf{Heatmap}
\\[1ex]
This heatmap is from rnn encoder decoder with attention mechanism, I extract the embedding layer and the hidden state from the encoder, and calculate the dot product between the weight matrix and the sentence vector. In this heatmap, each row corresponds to a hidden state dimension, and each column represents a word. 
\begin{figure}[!htb]
    \centering
    \includegraphics[width=1\textwidth]{../Pic/hidden_state.png}
    \caption{Heatmap of hidden state}
\end{figure}
\newline
Different dimensions in the hidden state activate for different words. For example, the words "highly" and "recommend" have bright activations in specific dimensions, indicating that these dimensinos are capturing the positive sentiment or recommendation context, the words like "is", "a" and "that" show weaker, less distinct activations, as they carry less information in the sentence. The word "must-watch" show the unique parttern, the most possible reason is that it is a compound word with significant semantic importance in the sentence. 

\newpage
\textbf{T-SNE visualization of hidden state}
\\[1ex]
This graph visualizaes the hidden states of the RNN encoder-decoder with attention after processing the sentence. The words are plotted in a 2 dimensional space by using t-SNE, which reduces the high dimensional hidden states to 2 dimensions while preserving their information.
\begin{figure}[!htb]
    \centering
    \includegraphics[width=1\textwidth]{../Pic/hidden_state_2.png}
    \caption{t-SNE}
\end{figure}
\newline
This plot is similar with the heatmap, is to show the relationship of how the words can be cluster together. The words like "highly", "recommend" and "great" are positioned close to each other, it is because they are all refer to the sentence's positive sentiment or recommendation context. Overall, the t-SNE plot shows how the model encodes the semantic and syntactic relationship between words. Words with similar meanings or roles in the sentence are encoded in similar regions of the hidden state space. 


\newpage
\section{Discussion and Conclusion}
\subsection{Results and Analysis}
The initial epochs are all models that started effective learning shown with approximately 55\% training and validation accuracy and the loss decreased consistently. A major problem that always arose across all models was overfitting. The primary reason behind the emergence of this phenomenon can be attributed to most of the dataset’s characteristics, especially truncation of sentences. The input to the models is restricted to the first 100 tokens and therefore, the models adjust to this input and may not fully capture the important context of each sentence. Furthermore the failure to generalize might be further loudened by the fact that the embedding matrix is defined using a built in layer in PyTorch with its values initialized to be randomly.

\subsection{Model Comparisons}
The three models analysed - vanilla RNN, LSTM and GRU, showed a superior performance of GRU and LSTM models. The underlying cause for this improvement however, is due to the limitation of the vanilla RNN at propagating long term dependencies (vanishing gradient problem) using 100 time steps of sequence processing. Its ability to effectively learn meaningful patterns of data allowed for this limitation. LSTM and GRU, on the other hand, utilize gating mechanism to retain relevant information and discard irrelevant information so that long term dependency can be captured more effectively.

\subsection{Overfitting Issues}
All of the three models were susceptible to the overfitting problem. However, as simple as the vanilla RNN is, it lacks advanced mechanisms of information retention and selective forgetting, and so instead of extracting the meaningful patterns embedded in the training data, it relies on memorizing it. Consequently, this results in overfitting. GRU and LSTM introduce gating mechanisms for data learning and are able to well learn from data, however, they do better optimization of training loss which can result to over fitting when they continue to learn unnecessary information from dataset.

\subsection{Deep Recurrent Neural Networks (DRNN)}
The performance of each model shows that the deep recurrent neural networks (DRNNs) is outperforms with the single layer models. When we stack more layer, then the model will become DRNNs and this architecture allows it to learn more complex and abstract representations of the data, in other word, the model able to capture or extract more useful information from the data. Thereby reducing the impact of overfitting.

\subsection{Attention Mechanism}
For the RNN encoder-decoder, I have applied attention mechanism, and among all of the models I have evaluated, the attention mechanism stood out as the most effective and the highest accuracy. The attention mechanism enhances the performance by allowing the model to focus on relevant parts of the input sequence, effectively retaining and utilizing crucial contextual information. 


\subsection{Gradient Problem}
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{../Pic/gradient.png}
\end{figure}
\subsubsection{Vanilla RNN (Red line)}
...
\\[1ex]
\subsubsection{LSTM (Blue line)}
The gradients of the LSTM are much more stable compared to the simple vanilla RNN. The gradient magnitudes remain consistent across timesteps and do not decay to zero. This stability is achieved through the use of gating machanisms that regulate the flow of information. These gates allow the LSTM to keep the relevant information form the input data while remove some irrelevant information from the data. As a result, LSTM is highly effective at learning long term dependencies in sequences.
\\[1ex]
\subsubsection{GRU (Green line)}
...
After I run all the model and compare their performance, serveral insights and challenges appear, which are discussed below.

\begin{itemize}
    \item \textbf{Unexpected Results:}  
    ...
    It is expected that the attention mechanism is outperformance comparing with other model since the attention mechanism is designed for capture the information from long sentence. So this is the expected outcome. 

    \item \textbf{Limitations:}  
    Due to limited computing resources, the experiments were constrained to training the models from scratch with relatively simpler architectures. This restricted the exploration of more complex or state-of-the-art models, such as Transformer-based architectures, which might offer further performance improvements but require substantially more computational power.
\end{itemize}
...
\begin{itemize}
    ...
    \\[1ex]
    Different RNN-based models, including vanilla RNN, LSTM, and GRU, were implemented and evaluated. The experiments show that LSTM and GRU models effectively address the long-term dependency problem inherent in sequential data. Their internal gating mechanisms allow them to control and preserve information over longer sequences.
    
    \item \textbf{Efficacy of Gated Architectures:}  
    \\[1ex]
    The results indicate that the gated architectures (LSTM and GRU) significantly outperform the vanilla RNN in terms of handling vanishing and exploding gradient issues, thereby offering more stable and reliable performance.

    \item \textbf{Importancy of attention mechanism}
    \\[1ex]
    We have plot the heatmap of the hidden state of the RNN encoder-decoder with attention mechanism, also i have plotted the t-SNE to show how the word be cluter together. And this 2 graph can illustrate how to attention mechanism train the model and train the parameter inside the model. 
\end{itemize}
