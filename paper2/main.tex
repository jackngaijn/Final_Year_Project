...
where the $f_{enc}$ is the recurrent transition function, (e.g. LSTM, GRU or Vanilla RNN cell), and $h_t$ is the hidden state at time step $t$. And then the final hidden state of the encoder, $h_T$ will act as the context vector.
\begin{equation}
    c = h_T
\end{equation}
\textbf{Decoder}
\\[1ex]
The decoder is the second component of RNN encoder decoder, the usage of the decoder is to generates the output sequence step by step, conditioned on the context vector $c$ and its previous hidden state. 
\begin{equation}
    s_t = f_{dec}(y_{t-1}, s_{t-1}, c)
\end{equation}
where $f_{dec}$ is the recurrent function of the decoder, and it is just like the encoder part, $y_{t-1}$ is the embedding of the previous output token, $s_{t-1}$ is the decoder's hidden state. And $c$ is the context vector from the encdoer, which remains constant throughout decoding. 
\\[1ex]
After the model computed the $s_t$, the model will compute the output probability distribution.
\begin{equation}
    P(y_t|y_1, c) = softmax(W_os_t+b_o)
\end{equation}
...
\\[1ex]
\begin{equation}
    e_{t',t} = v_a^Ttanh(W_as_{t'-1}+U_ah_t)
\end{equation}                    
